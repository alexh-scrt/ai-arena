...
class CognitiveCodaAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.coda_pattern = re.compile(
            r"(?:^|\n)\s*(?:Cognitive\s+)?Coda:\s*(.+)$", 
            re.IGNORECASE | re.MULTILINE
        )
        self.reasoning_pattern = re.compile(
            r"(?:^|\n)\s*(?:Reasoning\s+)?(?:Chain)?:\s*(.+)$", 
            re.IGNORECASE | re.DOTALL
        )
        # Fallback patterns for more flexible parsing
        self.coda_fallback_patterns = [
            re.compile(r"(?:^|\n)\s*Coda:\s*(.+)$", re.IGNORECASE | re.MULTILINE),
            re.compile(r"(?:^|\n)\s*Result:\s*(.+)$", re.IGNORECASE | re.MULTILINE),
            re.compile(r"(?:^|\n)\s*(.+)$", re.MULTILINE)  # First substantial line
        ]
    ...
    async def generate_coda(
        ...
        try:
            ...
            parsed = self._fallback_parse(raw_response)
            if not parsed:
                parsed = {
    ...
    def _parse_response(self, raw_text: str) -> Dict[str, str]:
        ...
        """
        logger.debug(f"Parsing response: {raw_text[:200]}...")
        
        # Extract coda
        coda_match = self.coda_pattern.search(raw_text)
        if not coda_match:
            # Try more flexible pattern
            flexible_coda = re.search(r"(?:coda|result|conclusion):\s*(.+)", raw_text, re.IGNORECASE)
            if not flexible_coda:
                raise ValueError("Could not find 'Cognitive Coda:' in response")
            coda_match = flexible_coda
        ...
        if not reasoning_match:
            # Try more flexible reasoning extraction
            flexible_reasoning = re.search(r"(?:reasoning|explanation|because):\s*(.+)", raw_text, re.IGNORECASE | re.DOTALL)
            if not flexible_reasoning:
                # Extract any subsequent text as reasoning
                ...
                if coda_line_idx and coda_line_idx + 1 < len(lines):
                    reasoning = '\n'.join(lines[coda_line_idx + 1:]).strip()
                else:
                    raise ValueError("Could not find 'Reasoning Chain:' in response")
            else:
                reasoning = flexible_reasoning.group(1).strip()
        else:
    ...
    def _fallback_parse(self, raw_text: str) -> Optional[Dict[str, str]]:
        """
        Attempt fallback parsing when standard format fails.
        
        Args:
            raw_text: Raw LLM output
            
        Returns:
            Dictionary with 'coda' and 'reasoning' or None if still failing
        """
        logger.info("Attempting fallback parsing...")
        
        # Try fallback coda patterns
        coda = None
        for pattern in self.coda_fallback_patterns:
            match = pattern.search(raw_text)
            if match:
                potential_coda = match.group(1).strip()
                # Check if it looks like a reasonable coda (not too long, substantial)
                if 3 <= len(potential_coda.split()) <= 20 and not potential_coda.startswith("The"):
                    coda = self._postprocess_coda(potential_coda)
                    break
        
        # If still no coda found, try to extract the first meaningful line
        if not coda:
            lines = [line.strip() for line in raw_text.split('\n') if line.strip()]
            for line in lines:
                # Skip obvious non-coda lines
                if (not line.startswith(('The', 'This', 'In', 'Here', 'Based'))
                    and 3 <= len(line.split()) <= 20
                    and not line.endswith(':')):
                    coda = self._postprocess_coda(line)
                    break
        
        # Extract reasoning (more flexible)
        reasoning = "Generated through fallback parsing."
        reasoning_match = self.reasoning_pattern.search(raw_text)
        if reasoning_match:
            reasoning = reasoning_match.group(1).strip()
        else:
            # Try to find any explanatory text
            lines = raw_text.split('\n')
            for i, line in enumerate(lines):
                if any(word in line.lower() for word in ['because', 'reveals', 'embodies', 'represents']):
                    reasoning = '\n'.join(lines[i:i+3]).strip()
                    break
        
        if coda:
            logger.info(f"Fallback parsing successful: {coda}")
            return {
                'coda': coda,
                'reasoning': reasoning
            }
        
        logger.warning("Fallback parsing also failed")
        return None
