...
Your topic introduction:"""
    ...
    async def introduce_participants(self, participants: List[ParticipantState]) -> str:
        ...
        for p in participants:
            # Check if participant has persona with backstory
            if hasattr(p, 'persona') and p.persona and isinstance(p.persona, dict) and 'backstory' in p.persona:
                persona = p.persona
                name = persona.get('name', p.name)
                backstory = persona.get('backstory', '')
                # Use backstory for richer introduction
                participant_intros.append(
                    f"- {name} ({p.get_pronouns()}): {backstory}"
                )
            else:
...
1. Introduces each by name with a brief description of their background and expertise
2. Highlights what unique perspective each brings based on their backstory
...
- Use the rich backstory information provided to make compelling introductions
