...
def get_available_panels():
    """Get list of available panel configurations"""
    panels_dir = Path(__file__).parent.parent / "config" / "panels"
    if not panels_dir.exists():
        return []
    return [f.stem for f in panels_dir.glob("*.yml")]
...
@click.option("--panel", type=click.Choice(get_available_panels()), help="Use a predefined panel")
...
def main(topic: str, file: str, depth: int, participants: int, panel: str, config: str, max_turns: int, narrator: bool,
    ...
    characters = characters.load_all_characters()
    # Display participants
    for p in participants_config:
        if p['persona'] and 'persona' in p:
            persona = characters.get(p['persona'])
            console.print(
