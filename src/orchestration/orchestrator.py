...
from src.controllers.creativity_controller import DynamicCreativityController, CreativityConfig
...
class MultiAgentDiscussionOrchestrator:
    ...
    def __init__(
        ...
        enable_dynamic_creativity: Optional[bool] = None
    ):
        # Load configuration
        config = TalksConfig()
        ...
        for config_item in participants_config:
            ...
            if 'persona' in config_item and config_item['persona']:
                try:
                    character_data = characters.load_character(config_item['persona'])
                    personality = character_data['archetype']
                except CharacterConfigurationError as e:
                    logger.error(f"Character loading error for {config_item['name']}: {e}")
        ...
        # Dynamic Creativity System (optional)
        if enable_dynamic_creativity is None:
            enable_dynamic_creativity = config.get('dynamic_creativity_adjustment.enabled', True)
        
        self.enable_dynamic_creativity = enable_dynamic_creativity
        self.creativity_controller = None
        
        if enable_dynamic_creativity:
            creativity_config_dict = config.get('dynamic_creativity_adjustment', {})
            creativity_config = CreativityConfig.from_config(creativity_config_dict)
            self.creativity_controller = DynamicCreativityController(creativity_config)
            logger.info(f"🎨 Dynamic creativity adjustment enabled")
    ...
    async def run_discussion(self, max_iterations: int = 30) -> List[Dict]:
        ...
        # Set max turns for creativity controller
        if self.enable_dynamic_creativity and self.creativity_controller:
            self.creativity_controller.set_max_turns(max_iterations)
        ...
        try:
            ...
            while self.group_state.turn_number < max_iterations:
                ...
                # Get dynamic creativity parameters if enabled
                dynamic_params = None
                if self.enable_dynamic_creativity and self.creativity_controller:
                    recommendation = self.creativity_controller.recommend_parameters(
                    ...
                    dynamic_params = {
                        ...
                        'frequency_penalty': recommendation.frequency_penalty
                    }
                    
                    # Show decision rationale at info level for visibility
                    logger.info(f"🎨 Dynamic adjustment for {speaker.state.name} (turn {self.group_state.turn_number + 1})")
                    logger.info(f"   📈 Recommended: T={recommendation.temperature:.2f}, top_p={recommendation.top_p:.2f}")
                    logger.info(f"   💭 Rationale: {recommendation.explanation}")
                else:
                    logger.debug(f"🎨 Dynamic creativity disabled for {speaker.state.name}")
                ...
                # Assess response for dynamic creativity adjustment
                if self.enable_dynamic_creativity and self.creativity_controller:
                    try:
                        assessment = self.creativity_controller.assess_response(
                            participant_id=next_speaker_id,
                            response=response,
                            turn_number=self.group_state.turn_number
                        )
                        logger.info(f"📊 Creativity assessment for {speaker.state.name}:")
                        logger.info(f"   🎯 Originality: {assessment.originality_score:.2f} | Quality: {assessment.quality_score:.2f}")
                        logger.info(f"   💬 {assessment.explanation}")
                        
                        # Highlight significant findings
                        if assessment.originality_score < 0.4:
                            logger.warning(f"   ⚠️  Low originality detected - next turn will boost creativity")
                        elif assessment.quality_score > 0.8:
                            logger.info(f"   ✨ High quality achieved - maintaining focus")
                            
                    except Exception as e:
                        logger.error(f"Creativity assessment failed for {speaker.state.name}: {e}")
